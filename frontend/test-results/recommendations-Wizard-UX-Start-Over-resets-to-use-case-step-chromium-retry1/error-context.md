...
- Name: recommendations.spec.ts >> Wizard UX >> Start Over resets to use case step
- Location: tests/recommendations.spec.ts:274:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByRole('button', { name: /start over/i }) resolved to 2 elements:
    1) <button title="Start over" class="ml-auto flex items-center gap-1.5 text-xs font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-1.5 transition-colors">🏠 Start Over</button> aka getByRole('button', { name: '🏠 Start Over' })
    2) <button class="w-full py-2.5 border-2 border-gray-300 dark:border-gray-700 rounded-xl text-sm font-semibold text-gray-600 dark:text-gray-300 hover:border-gray-400 dark:hover:border-gray-500 transition-colors">Start Over</button> aka getByRole('button', { name: 'Start Over', exact: true })

Call log:
  - waiting for getByRole('button', { name: /start over/i })
