...
3. ~~No automated tests yet for the new Next API routes.~~ Done — [`frontend/tests/cli-router-api.spec.ts`](./frontend/tests/cli-router-api.spec.ts) adds 16 Playwright API tests against `GET /api/cli-router/config` and `POST /api/cli-router/route`: response shape, catalog/thresholds contents, the Jev key never appearing in the response, default agent/cost-priority, content-type/JSON/array-body/oversized-body rejection, empty prompt, unknown agent/cost-priority, native-arg conflicts, `strict` type validation, strict-mode 422 on fallback, and cross-origin rejection. Run with `npx playwright test tests/cli-router-api.spec.ts` against a running `next dev`. `CliRouterPanel` itself (the React component) still has no test coverage.
